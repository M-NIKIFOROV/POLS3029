...
long <- data.frame(
  country = rep(tolower(raw[["COUNTRY"]]), times = length(year_cols)),
  ccode = rep(tolower(sub("\\..*$", "", raw[["SERIES_CODE"]])), times = length(year_cols)),
...
long <- long[order(long$ccode, long$year, long$indicator_type), ]

build_wide <- function(data_long, indicator_name) {
  subset_long <- data_long[data_long$indicator_type == indicator_name, c("country", "ccode", "year", "value")]

  wide <- reshape(
    subset_long,
    idvar = c("country", "ccode"),
    timevar = "year",
    direction = "wide"
  )
  ...
  ordered_names <- c("country", "ccode", as.character(sort(as.integer(year_names))))
  wide <- wide[ordered_names]
  wide[order(wide$ccode), ]
}
